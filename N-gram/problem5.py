...
if __name__ == '__main__':
	...
	# problem 5
	quest=['in the past','in the time', 'the jury said', 'the jury recommended', 'jury said that', 'agriculture teacher ,']
	...
	for i in range(len(quest)):
		print('p({} | {}, {}):\tunsmoothed: {},\tsmoothed: {}'.format(quest[i].split()[2],quest[i].split()[0],quest[i].split()[1],
			counts_tri[i]/cnt_bi_unsth[word_index_dict[quest[i].split()[0]]][word_index_dict[quest[i].split()[1]]],
			(counts_tri[i]+0.1)/(cnt_bi_unsth[word_index_dict[quest[i].split()[0]]][word_index_dict[quest[i].split()[1]]]+(0.1*len(word_index_dict)))))
